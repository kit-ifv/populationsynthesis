...
class ForestHierarchyGraph<T> : MutableHierarchyGraph<T>() {
    override fun addRelationship(child: T, parent: T) {
        if (parentGraph.containsVertex(child) && parentGraph.outgoingEdgesOf(child)
                .isNotEmpty()
        ) throw IllegalArgumentException("Child node $child already has a parent node, cannot overwrite to newParent=$parent")
